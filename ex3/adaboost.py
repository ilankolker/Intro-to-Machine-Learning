...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for t in range(self.iterations_):
            ...
            # Predict and compute the weighted error
            predictions = self.models_[-1].predict(X)
            misclassified = (predictions != y)
            epsilon_t = np.sum(self.D_[misclassified])

            # Calculate the Amount Of Say of this learner
            self.weights_[t] = 0.5 * np.log((1 / epsilon_t) - 1)

            # Update samples' distribution
            self.D_ *= np.exp(-y * self.weights_[t] * predictions)
            self.D_ /= np.sum(self.D_)  # Normalize
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for t in range(0, min(self.iterations_, T)):
            model_prediction = self.models_[t].predict(X)
            weighted_predictions += self.weights_[t] * model_prediction
