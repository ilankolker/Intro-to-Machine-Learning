...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        return -np.sum(y * (X @ self.weights) - np.log(1 + np.exp((X @ self.weights)))) / num_samples
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        # Jacobian of the function with respect to weights w:
        # ∇w f(w) = - (1/m) * Σ ^ m[ y_i * x_i - (exp(x_i^T w) / (1 + exp(x_i^T w))) * x_i ]
        exp_term = np.exp(X @ self.weights)
        return (-1 / np.shape(X)[0]) * ((y @ X) - (exp_term / (1 + exp_term)) @ X)
