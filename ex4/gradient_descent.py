...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        # Initialization
        delta, prev_weights = self.tol_, np.copy(f.weights_)
        summed_weights = np.zeros_like(f.weights_)
        best_weights, best_val = np.copy(f.weights_), f.compute_output(X=X, y=y)
        iterations = 0

        for t in range(self.max_iter_):
            # Calculate the gradient and value
            grad = f.compute_jacobian(X=X, y=y)
            eta = self.learning_rate_.lr_step()

            # Performing descent step
            f.weights_ -= eta * grad

            # Compute a new value of the updated weights
            val = f.compute_output(X=X, y=y)

            # Calculate delta, which is the Euclidean norm of w^(t)-w^(t-1)
            delta = np.linalg.norm(f.weights_ - prev_weights, ord=2)

            # Update the best_weights, best_val, prev_weights, summed_weights
            if val < best_val:
                best_val, best_weights = val, np.copy(f.weights_)
            summed_weights += f.weights_
            prev_weights = np.copy(f.weights_)

            # Call the callback function
            self.callback_(solver=self, weight=np.copy(f.weights_), val=val, grad=grad, t=t, eta=eta, delta=delta)
        ...
        # return the weights correspond to out type
        if self.out_type_ == "last":
            return f.weights_
        elif self.out_type_ == "best":
            return best_weights
        return summed_weights / iterations
